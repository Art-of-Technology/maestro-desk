...
> - **Web auto-deploy relay.** `.github/workflows/deploy-production-web.yml` sends `main` pushes touching `web/**` to the public API's `CRON_SECRET`-gated `POST /api/v1/cron/deploy-web` route. The API forwards the unchanged push payload to Dokploy using `DOKPLOY_WEB_DEPLOY_URL`. This relay is required because `paas.weez.boo` is LAN/VPN-only and cannot be reached by GitHub-hosted runners. Keep the Dokploy URL only in the API environment; GitHub needs only the existing `PROD_CRON_SECRET`.
