...
> - **Web auto-deploy relay.** `.github/workflows/deploy-production-web.yml` sends `main` pushes touching `web/**` (or the deployment workflow itself) to the public API's `CRON_SECRET`-gated `POST /api/v1/cron/deploy-web` route. Both push and manual runs construct a deployment payload with the ref, commit SHA, and a `web/index.html` watch-path marker. GitHub Actions omits commit file lists from push events, so forwarding its raw event makes Dokploy reject the `web/**` watch filter with HTTP 301. The marker signals an already-selected web deployment; it is not a file-change audit. The API forwards this payload to Dokploy using `DOKPLOY_WEB_DEPLOY_URL`. This relay is required because `paas.weez.boo` is LAN/VPN-only and cannot be reached by GitHub-hosted runners. Keep the Dokploy URL only in the API environment; GitHub needs only the existing `PROD_CRON_SECRET`. Failed runs show only allowlisted API error messages; check the API's **Swarm** logs in Dokploy for the upstream HTTP status.
> - **API auto-deploy relay.** `.github/workflows/deploy-production-api.yml` uses the same authenticated relay at `POST /api/v1/cron/deploy-api`, with `DOKPLOY_API_DEPLOY_URL` pointing to the private API webhook. Main pushes touching `api/**`, `db/migrations/**`, `.dockerignore`, or the workflow trigger it; the payload supplies an `api/Dockerfile` watch-path marker. Manual dispatch is main-only. Bootstrap once by setting the new URL and manually deploying the API, then run the workflow and verify the expected API fingerprint plus database readiness. If the API is down, use Dokploy directly; the relay is not a recovery mechanism. Migrations still run at container boot.
> - **Deployment failure alerts.** `.github/workflows/deployment-alerts.yml` watches completion of the API deploy, web deploy, and production health-check workflows on main. Failures and timeouts send a short message and run link to **#jodis-codex-alerts** using the repository secret `SLACK_DEPLOY_WEBHOOK_URL`. Successful, skipped, and cancelled runs (including superseded health checks) are silent. Delivery runs on GitHub, independent of the API and database. No commit messages, logs, customer data, or raw errors are included. Manual dispatch from main sends a clearly labelled test only; it does not deploy or simulate an outage. A failed Slack delivery fails the alert workflow visibly; there is no automatic retry because a timed-out POST may already have delivered. Check the secret/channel configuration if that workflow fails.
...
DOKPLOY_API_DEPLOY_URL=https://paas.weez.boo/api/deploy/…  # private respovia-api webhook
DOKPLOY_WEB_DEPLOY_URL=https://paas.weez.boo/api/deploy/…  # private respovia-web webhook; used only by the authenticated API relay
...
- **Automatic verification.** Every push to `main` runs `.github/workflows/post-deploy-healthcheck.yml`. The API version job requires the expected build fingerprint on both liveness and database readiness responses. The fingerprint is generated inside Docker from API source, locked dependencies, build configuration and migrations; frontend-only changes do not alter it. A previous container cannot pass for changed API inputs, including when the new image failed to build or boot. A separate job compares every tracked public frontend file with the expected checkout, excluding Docker/nginx/Vercel build configuration. Both retry during rollout. Inspect the failing job: a version mismatch can mean a missed deployment, not an outage. Matching fingerprints identify build inputs, not external package/base-image digests or runtime configuration. These checks observe deployments; they do not trigger them. Local development and legacy previews without a Docker-generated marker report a null fingerprint and cannot pass production version verification.
