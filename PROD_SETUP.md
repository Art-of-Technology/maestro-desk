...
- **Automatic verification.** Every push to `main` runs `.github/workflows/post-deploy-healthcheck.yml`. One job checks API availability and database connectivity. A separate job compares every tracked public frontend file with the expected checkout, excluding Docker/nginx/Vercel build configuration, and retries while deployment completes. A stale or mixed frontend fails verification even when the API is healthy. Inspect the failing job: a frontend mismatch can mean a missed deployment, not an outage. API health still does not establish the deployed API version or successful migration application. These checks observe deployments; they do not trigger them.
