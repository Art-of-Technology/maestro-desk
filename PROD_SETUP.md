...
Stack: **Supabase** (new prod project) · **Fly.io** (Bun/Hono API) · **Cloudflare Pages** (SPA) · **Postmark** (email). Replace `desk.example.com` / `help.example.com` / `support@example.com` with your real domain.

> Legend: 🤖 = Claude can do it (Supabase Management API / repo) · 👤 = you (billing, DNS, account auth).

## 1. Supabase prod project
- [ ] 👤 Create a new project (dashboard) — region **eu-central-1 (Frankfurt)**, set a strong DB password you keep. Pro recommended for daily backups + leaked-password protection; Free works to start. *(Or 🤖 Claude creates it via the Management API and you rotate the DB password.)*
- [ ] 🤖 `supabase link --project-ref <PROD_REF>` then `supabase db push` — applies all migrations to prod.
- [ ] 🤖 Set a real `site_url` + redirect URLs (Auth config) → `https://desk.example.com`.
- [ ] 👤 **Configure the Custom Access Token Hook** (Dashboard → Authentication → Hooks → `custom_access_token_hook`). **RLS depends on this** — without it the `workspace_ids` claim is missing and authenticated queries deny everything. (This is a dashboard toggle, not a migration.)
...
- [ ] 👤 Set secrets (values from prod Supabase + Anthropic + Postmark):
  ...
  fly secrets set \
    ...
    POSTMARK_SERVER_TOKEN=... POSTMARK_OUTBOUND_FROM=support@example.com \
    POSTMARK_ACCOUNT_TOKEN=... POSTMARK_INBOUND_REPLY_ADDRESS=...@inbound.postmarkapp.com \
    PORTAL_BASE_URL=https://help.example.com/portal.html
  ```
...
- [ ] 👤 (optional) Point `api.example.com` at the Fly app.

## 4. Frontend (Cloudflare Pages)
- [ ] 👤 Point the existing Pages project at prod: the SPA reads `GET /api/v1/config` for `supabase_url`/`anon_key`, so set the **API base URL** it calls to the Fly app (or `api.example.com`).
- [ ] 👤 Bind the domain → `desk.example.com` (app) and serve `portal.html` at `help.example.com`.
...
- [ ] 👤 Add DNS records on `example.com`:
...
## 6. Smoke + pilot
- [ ] 👤 Send a test email to `support@example.com` → confirm a ticket appears, auto-triage populates summary/draft.
- [ ] 👤 Agent logs in at `desk.example.com`, opens the ticket, replies → customer receives it from `support@example.com`.
