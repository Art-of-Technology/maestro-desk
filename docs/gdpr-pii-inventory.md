...
|---|---|---|---|
| `customers` | `first_name`, `last_name`, `username`, `email`, `mobile`, `backoffice_url`, `jurisdiction` | **null**; set `erased_at = now()` | Row kept (FKs from tickets). `display_id`, `brand`, `vip_tier`, `since`, `consent` retained as non-identifying / preference. `kyc_status` was removed from the product in Phase 4; the column is dropped separately and is no longer read or written. |
