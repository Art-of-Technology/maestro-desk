...
export default async(page)=>{
 ...
 for(const mode of ['normal','blocked','quota']){
  ...
  const result=await p.evaluate(async({user,mode})=>{
   ...
   t._uuid='storage-ticket';t._detailLoaded=true;
   ...
   (await import('/js/tickets/detail.js')).openTicket(t.id);await tx.ensureCustomerLanguage(t);await tx.toggleThreadTranslate(t.id,true);
