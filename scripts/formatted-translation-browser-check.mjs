...
export default async(page)=>{
 ...
 const result=await p.evaluate(async()=>{
  ...
  const tx=await import('/js/ai/translate.js');await tx.translateMessage(t.id,0);
  assert(t.msgs[0].translationHtml.includes('<strong>world</strong>'),'Individual message uses formatted translation');
  assert(document.querySelectorAll('iframe[data-msg-frame]').length===2,'Original and translated frames');
  ...
  tx.hideMessageTranslation(t.id,0);assert(!t.msgs[0].translationHtml,'Hide clears rich result');
  await tx.detectAndTranslateThread(t.id);
  ...
  const block=[...document.querySelectorAll('div')].find(e=>e.textContent==='Hello\n\nSecond paragraph');
  assert(block&&getComputedStyle(block).whiteSpace==='pre-wrap','Plain translation preserves line breaks');
