...
function helpFAQ() {
  const faqs = [
    ...
    {q:'How do I use placeholders in a response?', a:'In the Reply composer, choose <strong>Tools → Macros</strong>, then a canned response. In <strong>Preview response</strong>, check the automatically filled {name}, {ticket}, {brand} and {agent} values. Fill any missing fields, such as {transaction_reference}, with verified customer details. Choose <strong>Insert response</strong>, then review the whole draft. Inserting does not send it; replies with unfilled placeholders are blocked.'},
